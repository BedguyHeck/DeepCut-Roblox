...
/* -----------------------------
   GAME CARD RENDER (AI + LIBRARY SHARED)
------------------------------ */
...
async function renderGameCard(game) {
  ...
  img.src = "https://placehold.co/500x280/0f172a/38bdf8?text=Loading...";

  const placeId = extractPlaceId(game.link);

  if (placeId) {
    const thumb = await getThumbnail(placeId);
  ...
  content.innerHTML = `
    <h2>${game.title}</h2>
    <p class="desc">${game.reason || game.description || ""}</p>
...
form.addEventListener("submit", async (e) => {
  ...
  try {
    ...
    /* CASE 1: AI returns game recommendations */
    if (payload.games) {
...
/* -----------------------------
   STATIC GAME DATABASE (KEEP THIS)
------------------------------ */
const gameDatabase = {
  horror: [
    {
